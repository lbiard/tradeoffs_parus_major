...
for(i in 1:length(mi_df)) {
  ...
  X[[i]] <- as.matrix(cbind(rep(1, length(unique(df$BreedingSeason))),
                            ...
                            scale(unique(df[,c("BreedingSeason","population_density")])$population_density)[,1] * unique(mi_df[[i]][,c("BreedingSeason","mast.score")])$mast.score
  ))
...
setwd("/home/lbliar/data/great_tit/final_comments")

library(shinystan)
library(cmdstanr)

mod <- cmdstan_model("model_3_traits_ordinal.stan"
...
write.table(dat_plot, "posterior_draws_interaction.txt", append = FALSE, sep = " ", dec = ".",
